...
#   make CFLAGS=-O2
#   make install DESTDIR=$(CURDIR)/debian/dpl.lv2 PREFIX=/usr
#
PREFIX ?= /usr/local
BINDIR ?= $(PREFIX)/bin
...
dpl_VERSION ?= $(shell (git describe --tags HEAD || echo "0") | sed 's/-g.*$$//;s/^v//')
RW ?= robtk/
...
ifneq ($(BUILDJACKAPP), no)
 ...
 JACKAPP=$(APPBLD)x42-dpl$(EXE_EXT)
endif
...
ifneq ($(BUILDOPENGL), no)
	...
	sed "s/@LV2NAME@/$(LV2NAME)/g;s/@URISUFFIX@/mono/;s/@NAMESUFFIX@/ Mono/;s/@CTLSIZE@/65888/;s/@SIGNATURE@/$(SIGNATURE)/;s/@VERSION@/lv2:microVersion $(LV2MIC) ;lv2:minorVersion $(LV2MIN) ;/g;s/@UITTL@/$(UITTL)/" \
	    lv2ttl/$(LV2NAME).ports.ttl.in >> $(BUILDDIR)$(LV2NAME).ttl
	cat lv2ttl/$(LV2NAME).mono.ttl.in >> $(BUILDDIR)$(LV2NAME).ttl
	sed "s/@LV2NAME@/$(LV2NAME)/g;s/@URISUFFIX@/stereo/;s/@NAMESUFFIX@/ Stereo/;s/@CTLSIZE@/131424/;s/@SIGNATURE@/$(SIGNATURE)/;s/@VERSION@/lv2:microVersion $(LV2MIC) ;lv2:minorVersion $(LV2MIN) ;/g;s/@UITTL@/$(UITTL)/" \
	    lv2ttl/$(LV2NAME).ports.ttl.in >> $(BUILDDIR)$(LV2NAME).ttl
	cat lv2ttl/$(LV2NAME).stereo.ttl.in >> $(BUILDDIR)$(LV2NAME).ttl

DSP_SRC = src/lv2.cc src/peaklim.cc
DSP_DEPS = $(DSP_SRC) src/uris.h src/peaklim.h
GUI_DEPS = gui/dpl.c src/uris.h
...
$(eval x42_dpl_JACKSRC = -DX42_MULTIPLUGIN src/lv2.cc src/peaklim.cc)
x42_dpl_JACKGUI = gui/dpl.c
x42_dpl_LV2HTTL = lv2ttl/plugins.h
x42_dpl_JACKDESC = lv2ui_descriptor
$(APPBLD)x42-dpl$(EXE_EXT): $(DSP_DEPS) $(GUI_DEPS) \
	        $(x42_dpl_JACKGUI) $(x42_dpl_LV2HTTL)
...
uninstall-bin:
	...
	rm -f $(DESTDIR)$(BINDIR)/x42-dpl$(EXE_EXT)
...
ifneq ($(BUILDJACKAPP), no)
	...
	install -m644 x42-dpl.1 $(DESTDIR)$(MANDIR)
endif

uninstall-man:
	rm -f $(DESTDIR)$(MANDIR)/x42-dpl.1
	-rmdir $(DESTDIR)$(MANDIR)

man: $(APPBLD)x42-dpl
	help2man -N -o x42-dpl.1 -n "x42-dpl JACK Peak Limiter" $(APPBLD)x42-dpl
